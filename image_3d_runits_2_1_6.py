...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, seed=None):
        ...
        img_row_axis = self.row_axis - 2
        img_col_axis = self.col_axis - 2
        ...
        if augment:
            ...
            for r in range(rounds):
                for i in range(x.shape[0]):  # batch
                    for j in range(x.shape[1]):  # slice
                        ax[i + r * x.shape[0]][j] = self.random_transform(x[i][j])
            x = ax
...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        self.labels = image_data_generator.labels
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_y = np.zeros(tuple([len(index_array)] + list(self.x.shape)[1:]), dtype=K.floatx())
        for i, j in enumerate(index_array):
            batch_seed = np.random.randint(100)
            for k in range(self.x.shape[1]): #slice
                x = self.x[j][k]
               
                if self.image_data_generator.preprocessing_function:
                    x = self.image_data_generator.preprocessing_function(x)

                x = self.image_data_generator.random_transform(x.astype(K.floatx()), seed=batch_seed)
                x = self.image_data_generator.standardize(x)
                batch_x[i][k] = x

            lcs = []
            for landmark_pt in self.labels:
                # print('----r>', landmark_pt)
                y = self.y[j]
                ext_yy = (y == landmark_pt).astype(np.bool)

                idx = np.argwhere(ext_yy)
                # print('----r> pre', idx)

                for k in range(self.x.shape[1]):  # slice
                    yy = self.image_data_generator.random_transform(ext_yy[k].astype(K.floatx()), seed=batch_seed)
                    yy = self.image_data_generator.standardize(yy)
                    batch_y[i][k] = yy

                idx_post = np.unravel_index(batch_y.argmax(), batch_y.shape)
                lcs.append(idx_post)
                # print('----r> post', idx_post)
            lcs = np.vstack((lcs))
